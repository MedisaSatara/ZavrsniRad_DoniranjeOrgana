...
namespace DoniranjeOrgana.Services.Service
{
    ...
    {
        public BaseCRUDService(DoniranjeOrganaContext eContext, IMapper mapper) : base(eContext, mapper)
        {
        }
        ...
        public virtual T Delete(int id)
        {
            ...
            if (entity != null)
            {
                ...
                {
                    var relatedDoktori = _context.Doktors.Where(d => d.SpecijalizacijaId == id).ToList();

                    if (relatedDoktori.Any())
                    {
                        foreach (var doktor in relatedDoktori)
                        {
                            doktor.SpecijalizacijaId = null;
                        }
